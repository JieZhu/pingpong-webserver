        ...
        if (select_retval > 0) /* at least one file descriptor is ready */
        {
            ...
            for (current = head.next; current; current = next) {
                ...
                if (FD_ISSET(current->socket, &write_set)) {
                    ...
                    count = send(current->socket, current->sendBuf, current->BUF_LEN, MSG_DONTWAIT);
                    if (count < 0) {
                        if (errno == EAGAIN) {
                ...
                if (FD_ISSET(current->socket, &read_set)) {
                    ...
                    if (count <= 0) {
                        ...
                        //fist 2 bytes indicate the size;
                        int size = ntohl( *(short*) buf );
                        ...
                        if (size+1 != count) {
                            ...
                            current->BUF_LEN = size;
                            current->sendBuf = malloc(size);
                            current->sendBuf = (char *)(buf+10);
